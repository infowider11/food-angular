            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Meal Type List</h4>
                        <a href="javascript:void(0);" class="btn btn-primary mb-1" data-bs-toggle="modal" data-bs-target="#addMealModal">Add Meal Type</a>
                    </div>
                    <div class="card-body">
                                    ...
                                    if(!empty($meal)){
                                        ...
                                        foreach ($meal as $key => $val) { ?>
                                            <tr>
                                                ...
                                                <td><p><?= $val['title'] ?></p></td>
                                                <td>
                                                    <a href="javascript:;" class="btn btn-primary shadow btn-xs sharp me-1" data-bs-toggle="modal" data-bs-target="#updateMealModal<?= $val['id']; ?>"><i class="fas fa-pencil-alt"></i></a>
...
<div class="modal fade" id="updateMealModal<?= $val['id']; ?>">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form method="post" onsubmit="return updatMeal(this, event, <?= $val['id']; ?>)" id="updatMeal" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="mb-3 col-md-12">
                                ...
                                <input type="text"  name="title" placeholder="name.." class="form-control" value="<?= $val['title'] ?>" required>
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    $('.alert-danger').remove();
        ...
        $.ajax({
              url: '<?= base_url()?>Admin/Meal/add_Meal_type',
              data: data,
              processData: false,
              contentType: false,
              type: 'POST',
  ...
  function updatMeal(el, event, id) {
    event.preventDefault();
    $('.alert-danger').remove();
    var data = new FormData($(el)[0]);

    $.ajax({
        url: '<?= base_url()?>Admin/Meal/update_meal_type',
        data: data,
        processData: false,
        contentType: false,
        type: 'POST',
    ...
    if(confirm('Are you sure ?')){
      $.ajax ({
          url: '<?= base_url()?>Admin/Meal/delete_meal_type',
          data: {id:id},
          type: 'POST',
          dataType:'json',
          beforeSend: function() {        
